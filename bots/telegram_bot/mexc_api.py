...
- Quyết định lệnh:
  ...
  • Ngược lại → LIMIT theo ATR(5m): Entry zone đón hồi, TTL 15 phút, TP/SL theo ATR.
...
- Hiển thị VND kiểu ONUS, **truncate (không làm tròn lên)**:
  ≥100k: 0 lẻ; ≥1k: 2 lẻ; <1k: 4 lẻ (hợp coin siêu nhỏ như PEPE, SHIB…).
"""
...
try:
    from settings import (
        # API endpoints
        MEXC_TICKER_URL, MEXC_FUNDING_URL, MEXC_KLINES_URL,  # "...kline?symbol={sym}&interval=Min1&limit=120"
        USDVND_URL,
        # HTTP
    ...
    USDVND_URL       = "https://open.er-api.com/v6/latest/USD"

    HTTP_TIMEOUT = 8
    HTTP_RETRY   = 2
    FX_CACHE_TTL = 10  # giây

    VOL24H_FLOOR = 200_000  # USDT
...
_hist_px: Dict[str, deque] = {}              # giá USD sau auto-denom, lưu 3 mốc để ước r30/r60
...
def _fetch_klines_1m(sym: str) -> List[dict]:
    ...
    for r in rows:
        try:
            t = int(r[0]) // 1000 if isinstance(r[0], (int, float)) else int(r.get("time", 0)) // 1000
...
def market_snapshot(unit: str = "VND", topn: int = DIVERSITY_POOL_TOPN) -> Tuple[List[dict], bool, float]:
    ...
    for it in items:
        ...
        d = {
            "symbol": sym,
            "displaySymbol": disp,
            "lastPrice": adj_usd,                 # USD sau denom
...
def _build_reason(strategy: str, ctx: dict) -> str:
    """
    strategy: "MARKET" hoặc "LIMIT"
    ctx: r30, r60, accel, vol_mult, funding, ema9, ema21, rsi14
    -> Viết lý do kiểu MA/RSI dễ hiểu
    """
    ...
    if strategy == "MARKET":
        return (
            ...
            f"• Volume hiện tại = {ctx['vol_mult']:.2f}× MA20Vol (xác nhận), Funding {ctx['funding']:+.3f}%\n"
...
def smart_pick_signals(unit: str, n_scalp: int = 5):
    ...
    # Cập nhật lịch sử giá để tính r30/r60
