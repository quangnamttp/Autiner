...
import { makeFiveOnusSignalsLive, getOnusTop5GainersInstant } from '../signals/fiveMaker.js';
...
export function startSchedulers() {
  // 06:00 — chào sáng (top tăng chụp ngay tại lúc 06:00)
  cron.schedule('0 6 * * *', async () => {
    if (!CHAT_ID) return;
    const top5 = await getOnusTop5GainersInstant();
  ...
  cron.schedule('0 7 * * *', async () => {
    if (!CHAT_ID) return;
    await sendMessage(CHAT_ID, '📅 07:00 Lịch vĩ mô (sẽ kết nối ForexFactory).');
  }, { timezone: TZ });

  // 06:15 → 21:45 — mỗi 30’ : QUÉT LIVE → CHỌN 5 → GỬI NGAY
  cron.schedule('15,45 6-21 * * *', async () => {
    ...
    const sigs = await makeFiveOnusSignalsLive(); // live multi-scrape, gộp, chọn 5
